...
android {
    namespace = "com.example.omdb_kotlin"
    compileSdk = 34

    defaultConfig {
        applicationId = "com.example.omdb_kotlin"
        minSdk = 30
        targetSdk = 34
        versionCode = 1
        versionName = "1.0"
        ...
        // Expose OMDb API key to BuildConfig from env var, Gradle property, or .env
        val omdbKey = (
                ...
                ?: run {
                    ...
                    if (f.exists()) {
                        val line = f.readLines()
                            .firstOrNull { it.trim().startsWith("OMDB_API_KEY=") }
                        line?.substringAfter("OMDB_API_KEY=")?.trim()
                    } else null
                }
